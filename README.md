...
- `winclada.ss` includes data matrix and consensus tree and it can be read by winclada directly. (The default of TNT treats all character nonadditve `ccode - 0 . ntax-1;`, however, Winclada treats all character additive `ccode + 0 . ntax-1;`, to fix this, you should add `ccode` relatd code in the `winclada.ss`, before `proc/;` )
